...
if __name__ == '__main__':
    ...
    def make_path(path, x):
        return str(Path(path, x.filename))


    logging.basicConfig(filename='training_without_padding_50.log', level=logging.INFO)

    df = pd.read_csv(r"I:\accent_kaggle\cv-valid-train.csv")
    df = df[["filename", "accent"]]
    df.dropna(inplace=True)
    df = df[df.accent != "other"]
    df = df[df["accent"].isin(["us", "indian", "england", "canada", "australia"])]
    df = df.sample(frac=1).reset_index(drop=True)

    # unique = df["accent"].unique()
    path = Path(r"I:\accent_kaggle\cv-valid-train")
    df["filename"] = df.apply(lambda x: make_path(path, x), axis=1)
    df = df.sample(frac=1)
    ...
    X_features = df["filename"].to_numpy()
    ...
    X_val = pool.map(get_wav, X_val)

    # Convert to MFCC
    if DEBUG:
        print('Converting to MFCC....')
    X_train = pool.map(to_mfcc, X_train)
    X_val = pool.map(to_mfcc, X_val)

    # Create segments from MFCCs
    X_train, y_train = make_segments(X_train, Y_train)
    X_validation, y_validation = make_segments(X_val, Y_val)
